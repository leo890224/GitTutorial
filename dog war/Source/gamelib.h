...
namespace game_framework
{
	...
	class CAnimation
	{
		...
		private:
			...
			list<CMovingBitmap>::iterator	bmp_iter;		// list
	...
	{
		public:
			CInteger(int = 2);			// default 2 digits
			void LoadBitmap();			// 載入0..9及負號之圖形
			void SetInteger(int);		// 設定整數值
			void SetTopLeft(int, int);	// 將動畫的左上角座標移至 (x,y)
			void ShowBitmap(int, int, int, int, int);			// 將動畫貼到螢幕
			void ShowMax(int);
			void ShowNum(int);
		private:
			...
			static CMovingBitmap base_digit[10];
			static CMovingBitmap upgrade_digit[10];
			CMovingBitmap slash;
			CMovingBitmap slash2;
			CMovingBitmap won;
			CMovingBitmap Lwon;
			int x, y;						// 顯示的座標
			int n;							// 整數值
